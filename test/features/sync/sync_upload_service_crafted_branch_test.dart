...
void main() {
  ...
  group('Crafted Audio Cloud Sync branch', () {
    test('crafted row sends signedId in JSON payload when uploader returns it',
        () async {
      ...
      // finds the bytes.
      final mediaDir = Directory(p.join(docsRoot.path, 'media'))..createSync();
    ...
    test('provider=youtube row does not trigger the uploader (SC-003)', () async {
      var directUploadsCalled = false;
