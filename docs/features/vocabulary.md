...
- Back: smaller serif headword + Pronounce icon; compact bordered pill tabs (**Notes hidden until implemented**):
  - **Context** — quote with left rule and word highlight; **context pager** when the item has 2+ contexts; one muted source line + locator; compact icon row (tooltips) for Play segment / Open in player / Echo; contextual translation without a heavy heading. Flip, rate, and session transitions stop any in-flight pronounce playback.
...
- **Ratings live in the session footer**, not inside the card, and only after flip: Don't know / Know / Know well using score-bad / score-warn / score-good container tokens (44px min height). Desktop rating shortcuts 1/2/3 still require the card to be flipped.
...
| File | Role |
|------|------|
| [`vocabulary_flashcard.dart`](../../lib/features/vocabulary/presentation/vocabulary_flashcard.dart) | Top-level `VocabularyFlashcard` with front/back flip and scrollable tab body |
| [`widgets/vocabulary_rating_bar.dart`](../../lib/features/vocabulary/presentation/widgets/vocabulary_rating_bar.dart) | `VocabularyRatingBar` — session-level Don't know / Know / Know well chips |
| [`widgets/vocabulary_flashcard_context_tab.dart`](../../lib/features/vocabulary/presentation/widgets/vocabulary_flashcard_context_tab.dart) | `FlashcardContextTab` — context quote, source line, compact media actions, contextual translation |
