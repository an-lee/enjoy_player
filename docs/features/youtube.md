...
- **Windows play startup**: `flutter_inappwebview`'s `mediaPlaybackRequiresUserGesture` setting is not implemented by its WebView2 backend. Programmatic play therefore starts muted, waits for the authoritative HTML5 `playing` event, and restores the configured volume after a short initial settle window; later pause/resume cycles restore volume immediately. The earlier flow force-unmuted on the optimistic `play` event, which could make YouTube immediately pause before frames advanced. A `play` event alone no longer marks the app transport as playing; rejected `play()` promises are logged. Clicking the video pane must reach WebView2 — host chrome overlays use `MouseRegion(opaque: false)` so empty regions do not swallow that gesture.
