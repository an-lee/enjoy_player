...
- **Play startup (all platforms, Windows especially)**: `flutter_inappwebview`'s `mediaPlaybackRequiresUserGesture` setting is not implemented by its WebView2 backend. Programmatic play therefore starts muted and waits for the authoritative HTML5 `playing` event. First-play volume restore is **progress-gated** (unmute only after `<video>.currentTime` advances, with a ~1.5 s fallback); later pause/resume cycles restore volume immediately. A `play` event alone never marks the app transport as playing; rejected `play()` promises clear buffering and surface the localized tap-to-play hint. Transport toggle uses an atomic DOM-aware `playOrPause` script so a stale Dart `playing=true` cannot issue another `pause()` while the element is already paused. Explicit user/app play cancels the 6 s nudge and defers stall reloads. **All transport and volume commands prefer the page's own player object** (`#movie_player` / `.html5-video-player` — `playVideo` / `pauseVideo` / `mute` / `unMute` / `setVolume`) and only fall back to raw `<video>` mutation when that API is missing: flipping `video.muted=false` directly on the element let YouTube's gesture-gated autoplay policy re-pause the video ~0.5 s after playback starts (the recurring play→pause bug). As a safety net, when an explicit user play is confirmed paused almost immediately after starting, the poll loop re-issues play exactly **once** (transport decisions D8); a second immediate pause surfaces the recovery hint instead of looping. Clicking the video pane must reach the WebView — host chrome overlays use `MouseRegion(opaque: false)` / `IgnorePointer` so empty regions do not swallow that gesture.
...
3. Check diagnostic logs for `youtube play command` / `youtube playOrPause command`, `youtube video play requested`, `youtube video playing`, `youtube play rejected`, `youtube volume restored`, `youtube pause confirmed`, `youtube immediate pause`, `youtube immediate pause retry`, `youtube init load_stop`, `youtube playback stalled`, or `WebView process terminated`. Enable **Settings → About → Diagnostic logging** before reproduction to include the FINE-level command, event, and poll-transition records. Smoking-gun pattern for the intermittent play→pause bug: `playing` → (optional `volume restored`) → `immediate pause` within ~2 s. Since transport now routes through the page player object and grants one automatic `immediate pause retry`, a healthy recovery reads `immediate pause` → `immediate pause retry` → `video playing`; a persistent failure repeats `immediate pause` without a second retry.
